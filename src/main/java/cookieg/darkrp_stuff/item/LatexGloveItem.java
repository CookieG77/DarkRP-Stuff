...
import cookieg.darkrp_stuff.procedures.InspectPlayerProcedure;
import cookieg.darkrp_stuff.itemgroup.DarkRPStuffItemGroup;
import cookieg.darkrp_stuff.gui.InspectGuiWindow;
...
public class LatexGloveItem extends DarkrpStuffModElements.ModElement {
	@ObjectHolder("darkrp_stuff:latex_glove")
	public static final Item block = null;

	public LatexGloveItem(DarkrpStuffModElements instance) {
		super(instance, 167);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof InspectGuiWindow) {
	...
	public static class ItemCustom extends Item {
		...
		public void addInformation(ItemStack itemstack, World world, List<ITextComponent> list, ITooltipFlag flag) {
			...
			list.add(new StringTextComponent("Permet de fouiller le joueur le plus proche."));
		}
		...
		public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity entity, Hand hand) {
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("Gant en Latex");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new InspectGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
			...
			InspectPlayerProcedure.executeProcedure(Stream
